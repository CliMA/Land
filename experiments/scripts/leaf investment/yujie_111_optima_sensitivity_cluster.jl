# include("./scripts/leaf investment/yujie_111_optima_sensitivity_cluster.jl")
...
for i in [4,7,8,20]
    ...
    GenerateOptima(weat_his_amb, year_his, "./data/" * site * "/amb_def.txt", lati, long, alti, "wb", 1.0)
    ...
    for lab in ["ca", "cc", "cv", "ga", "gm", "kl", "kw", "rh", "sd", "ta", "wb", "wc", "wk"]
        ...
        if lab=="ca"
            for ratio_to_vary in [0.75,1.5,2.0]
                println("Now running its ", ratio_to_vary, "X the default")
                file_to_write = "./data/" * site * "/amb_" * lab * "_" * string(ratio_to_vary) * ".txt"
                GenerateOptima(weat_his_amb, year_his, file_to_write, lati, long, alti, lab, ratio_to_vary)
            end
        elseif lab=="rh"
            for ratio_to_vary in [0.8,1.2,1.4]
                println("Now running its ", ratio_to_vary, "X the default")
                file_to_write = "./data/" * site * "/amb_" * lab * "_" * string(ratio_to_vary) * ".txt"
                GenerateOptima(weat_his_amb, year_his, file_to_write, lati, long, alti, lab, ratio_to_vary)
            end
        elseif lab=="ta"
            for ratio_to_vary in [-2,2,4]
                if ratio_to_vary<0
            ...
            for ratio_to_vary in [0.5,1.5,2.0]
                println("Now running its ", ratio_to_vary, "X the default")
                file_to_write = "./data/" * site * "/amb_" * lab * "_" * string(ratio_to_vary) * ".txt"
                GenerateOptima(weat_his_amb, year_his, file_to_write, lati, long, alti, lab, ratio_to_vary)
            end
