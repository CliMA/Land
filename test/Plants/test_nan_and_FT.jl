    ...
    for FT in [Float32 Float64]
     ...
     curvature::FT = FT(0.9)
        ...
        for tmp in [PLT.get_p_base_q_list_from_q(tree, FT(1.0)),
                    PLT.get_q_layer_from_p_base(tree.roots.root_list[1], FT(-0.1)),
                    PLT.get_struct_p_end_from_q(tree.roots.root_list[1], FT(0.1); p_ini=FT(Inf)),
                    PLT.get_struct_p_end_from_q(tree.trunk, FT(1.0); p_ini=FT(Inf)),
                    PLT.get_struct_p_end_from_q(tree.branch.branch_list[1], FT(0.1); p_ini=FT(Inf)),
                    PLT.get_struct_p_end_from_q(tree.canopy.canopy_list[1].leaf_list[1], FT(0.05); p_ini=FT(Inf)),
                    PLT.get_marginal_gain(tree.canopy.canopy_list[1], 1, tree.photo_para_set),
                    PLT.get_marginal_gain(tree.canopy.canopy_list[1], tree.photo_para_set),
                    PLT.get_marginal_penalty_wang(tree.canopy.canopy_list[1], 1),
                    PLT.get_marginal_penalty_wang(tree.canopy.canopy_list[1]),
