...
Base.@kwdef mutable struct StructTreeRootLayer{FT<:AbstractFloat}
    # root structure
    ...
    t_soil   ::FT     = FT(298.15  )         # K   | soil temperature
    ...
    t_element::Array{FT,1} =  ones(FT,10) * FT(298.13)    # K             | a list of stem temperature for each element
...
function create_root_list(FT, n=5, z_lo=-1.0)
    ...
    root_list = [StructTreeRootLayer{FT}() for i in 1:n]
...
Base.@kwdef mutable struct StructTreeRoot{FT<:AbstractFloat,n}
    ...
    root_list::Array{StructTreeRootLayer{FT},1} = create_root_list(FT,n)             # | a list of root layer
end
