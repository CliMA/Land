...
#
###############################################################################
""" ρ_H₂O * GRAVITY * 1e-6 * _h gives MPa """
const _Rho_G_6 = ρ_H₂O * GRAVITY * 1e-6
""" An array for splitted rhizosphere conductance """
const _K_RHIZ_ARRAY = log( 10 ) ./ log.( (100 .- 9 .* collect(0:9)) ./ (91 .- 9 .* collect(0:9)) )
...
"""
function get_struct_p_end_from_q(root_layer::RootLayer{FT}, flow::FT; p_ini::FT=FT(Inf)) where {FT<:AbstractFloat}
    @unpack b,c,k_element,k_rhiz,p_element,p_history,p_ups,soil_a,soil_m,soil_n,t_element,t_soil,z_element = root_layer
    ...
    # compute the pressure drop along the rhizosphere, considering surface tension
    # TODO further switvh to use soil module functions?
    _ks = relative_surface_tension(t_soil)
    _dp = flow / k_rhiz * relative_viscosity(t_soil)
    for _kr in _K_RHIZ_ARRAY
        if p_end<=0
            shell_t = (1 / (1 + (soil_a*(-p_end/_ks))^soil_n)) ^ soil_m
            shell_f = max( FT(1e-20), sqrt(shell_t) * (1 - (1-shell_t^(1/soil_m)) ^ soil_m) ^ 2 )
        else
            shell_f = 1
        end
        p_end -= _dp / shell_f / FT(_kr)
    end

    # compute k from temperature and pressure (history), then update pressure
    for (_k, _ph, _t, _z) in zip(k_element, p_history, t_element, z_element)
        k      = weibull_k_ratio(b, c, p_end, _t, _ph) * _k
        p_end -= flow / k + FT(_Rho_G_6) * _z
    end

    return p_end
end

function get_struct_p_end_from_q(stem::Stem{FT}, flow::FT; p_ini::FT=FT(Inf)) where {FT<:AbstractFloat}
    @unpack b,c,k_element,p_history,t_element,z_element = stem
...
function update_leaf_ak_max!(tree::Tree)
    ...
    for canopyi in canopy_list
        _leaf = LeafPhotoContainer{FTYP}(Vcmax25  = canopyi.v_max,
                                         Jmax25   = canopyi.j_max,
                                         Vpmax25  = canopyi.p_max,
                                         Rd25     = canopyi.r_25)
        _env  = EnvironmentConditions{FTYP}(p_a   = canopyi.p_a,
                                            p_atm = canopyi.p_atm,
                                            p_O₂  = canopyi.p_O₂)
        ...
        for i in 1:length(canopyi.leaf_list)
            ...
            glw  = min( e_c / (canopyi.d_list[i] / canopyi.p_atm), canopyi.g_max )
            glc  = glw / FTYP(1.6) / ( 1 + canopyi.g_ias_c * glw^(canopyi.g_ias_e) )
            # TODO restruct the CanopyLayer including an EnvironmentConditions paraset
            _leaf.T    = canopyi.t_list[i]
            _leaf.APAR = canopyi.par_list[i]
            leaf_photosynthesis_from_glc!(photo_para_set, _leaf, _env, glc)
            a_m  = leaf.An
