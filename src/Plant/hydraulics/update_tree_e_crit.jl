...
function update_tree_e_crit(tree, displaying=false)
    while true
        # define judge = 0
        judge = 0
        ...
        p_trunk_branch = get_struct_p_end_from_q(tree.trunk, q_sum, p_base)
        ...
        for indx in 1:length(tree.branch.branch_list)
            ...
            p_branch_leaf = get_struct_p_end_from_q(branchi, q_branch_list[indx], p_trunk_branch)

            # compute p_leaf for each leaf
            for leaf in canopyi.leaf_list
                p_leaf = get_struct_p_end_from_q(leaf, leaf.e_crit, p_branch_leaf)
