...
function update_tree_with_time!(tree::Tree, Δt::FT, scheme::ESMGentine; updating::Bool=false) where {FT}
    ...
    for canopyi in canopy_list
        ...
        k_ratio = ones(length(canopyi.leaf_list))
        for indx in 1:length(canopyi.leaf_list)
            leaf   = canopyi.leaf_list[indx]
            k_rati = exp( -(-leaf.p_dos / leaf.b)^leaf.c )
            k_ratio[indx] = k_rati
        end
        gss_list = get_empirical_gsw_from_model.(scheme, canopyi.an_list, k_ratio, canopyi.p_atm, canopyi.pi_list)
...
function update_tree_with_time!(tree::Tree, Δt::FT, scheme::AbstractEmpiricalStomatalModel; updating::Bool=false) where {FT}
    ...
    for canopyi in canopy_list
        ...
        rh  = canopyi.p_H₂O / get_saturated_vapor_pressure(canopyi.t_air)
        vpd = get_saturated_vapor_pressure.(canopyi.t_list) .- canopyi.p_H₂O
        gss_list = get_empirical_gsw_from_model.(scheme, canopyi.an_list, canopyi.p_atm, canopyi.pi_list, rh, canopyi.d_list, Γ_star)
