...
function van_Genuchten(params, psi)
    ...
    # added Gentine, m is known as a function of n
    m = 1-1/n

    # --- Relative saturation [Se] for specified matric potential [psi]
    if (psi < 0)
    ...
    # --- dtheta_dpsi required to copmute diffusivity of head based tepomral Richards' equation
    if (psi < 0)
        dtheta_dpsi = (theta_sat - theta_res) * m*n*alpha^n*abs(psi)^(n-1)*(1 + (alpha * abs(psi))^n)^(-m-1)
    else()
        dtheta_dpsi = 0
    end

    return theta, K, cap, dtheta_dpsi
...
function predictor_corrector_moisture(soil::soil_struct, params, G0::Float64, dt)
  ...
  # soil.Ts              ! Soil temperature [K]
  #
  # Output
  # soil.dTs          ! Change in soil temperature [K]
  # soil.err             ! Energy balance error (W/m2)
  ...
  for i = 1:soil.nsoi
     soil.theta[i], soil.K[i], soil.cap[i] = HeatDiffusion(params, theta_n)
  end
...
function predictor_corrector_moisture(soil::soil_struct, params, ET::Float64, dt)
...
function compute_grid_settings(soil::soil_struct)
   ...
   for i = 1:soil.nsoi
      soil.dz[i] = soil.zsoil/Float64(soil.nsoi)
   end
