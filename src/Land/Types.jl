...
Base.@kwdef mutable struct VerticalLayers{FT}
    ...
    n_layer::Int = 20
    "Thickness per layer"
    d_layer::Vector{FT} = FT[1 for i in 1:n_layer]
    "Vertical distances among layers (0.5 for boundaries)"
    x_layer::Vector{FT} = FT[0.5; [1 for i in 1:n_layer-2]; 0.5]

    # Temperature
    "Temperature at each layer"
    T::Vector{FT} = ones(FT,n_layer) .* T_25(FT)

    # CO₂ partial pressure
    "Diffusion coefficient array"
    d_CO₂::Vector{FT} = diffusive_coefficient.(T,
                                                [TraceGasCO₂()],
                                                [TraceGasAir()])
